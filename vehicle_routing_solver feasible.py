...
#            print()
    ...
    def makeTruckRoutes(self):
        ...
        for request in self.Instance.Requests:
            ...
            ID = len(schedule.TruckRoutes) + 1

            if ID <= len(self.Trucks):
                truck = self.Trucks[ID - 1]
            else:
                truck = self.Truck(ID)
                self.Trucks.append(truck)
                
            truckDaySchedule = self.TruckDaySchedule(deliveryDay)
            truckDaySchedule.RequestIDs.append(request.ID)
            truckDaySchedule.Distance = 2 * self.Instance.calcDistance[0][request.customerLocID - 1]
            truck.TruckDaySchedules[deliveryDay] = truckDaySchedule
            
            route = self.TruckRoute(ID)
            route.RequestIDs = [ request.ID ]
            schedule.TruckRoutes.append(route)
